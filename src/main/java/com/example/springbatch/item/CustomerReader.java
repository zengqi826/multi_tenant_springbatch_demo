 ...
 *
 * <p>
 * It is important to note that this is a paging item reader and exceptions that
 * are thrown while reading the page itself (mapping results to objects, etc in
 * the {@link CustomerReader#doPageRead()}) will not be skippable since this
 * reader has no way of knowing if an exception should be skipped and therefore
 * will continue to read the same page until the skip limit is exceeded.
 * </p>
 *
 * <p>
 * NOTE: The {@code RepositoryItemReader} only reads Java Objects i.e. non
 * primitives.
 * </p>
 *
 ...
 * @since 2.2
 */
public class CustomerReader<T> extends AbstractItemCountingItemStreamItemReader<T> implements InitializingBean {
    ...
    public CustomerReader() {
        setName(ClassUtils.getShortName(CustomerReader.class));
    }
    ...
    /**
     * Provides ordering of the results so that order is maintained between paged
     * queries
     *
     * @param sorts
